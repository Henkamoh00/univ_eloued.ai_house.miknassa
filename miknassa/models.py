...
class User(db.Model, UserMixin):
    ...
    r_trucksReport = db.relationship(
        "TrucksReport", backref="reporter", lazy=True, cascade="all, delete-orphan"
    )
    r_truck = db.relationship(
        "Truck", backref="driver", lazy=True, cascade="all, delete-orphan"
    )
...
class Wilaya(db.Model):
    ...
    r_dayra = db.relationship(
        "Dayra", backref="dayraIn", lazy=True, cascade="all, delete-orphan"
    )

    def __repr__(self):
        return (
            f"{{'id': {self.id}, 'name': {self.name}, 'matricule': {self.matricule}}}"
        )
...
class Dayra(db.Model):
    ...
    r_municipality = db.relationship(
        "Municipality",
        backref="municipalityIn",
        lazy=True,
...
class UserType(db.Model):
    ...
    r_user = db.relationship(
        "User", backref="accountType", lazy=True, cascade="all, delete-orphan"
    )
...
class Truck(db.Model):
    ...
    isActive = db.Column(db.Boolean, nullable=False, default=False, server_default='0')

    r_trucksReport = db.relationship(
        "TrucksReport", backref="reportOn", lazy=True, cascade="all, delete-orphan"
    )
    r_operation = db.relationship(
        "Operation", backref="performer", lazy=True, cascade="all, delete-orphan"
    )
...
class TruckType(db.Model):
    ...
    r_truck = db.relationship(
        "Truck", backref="truckType", lazy=True, cascade="all, delete-orphan"
    )
